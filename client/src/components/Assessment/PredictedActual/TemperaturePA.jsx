...
const TemperaturePA = () => {
  ...
  const predictedReadingColor = "#8884d8"; 
  const actualReadingColor = "#6F93D3"; 
  ...
  const findLastFiveConsecutiveMatches = (data) => {
      for (let i = data.length - 1; i >= 4; i--) {
          if (data.slice(i-4, i+1).every(item => 
                item.allDataValue !== null)) 
        {
          return i - 4;
            }
      }
  ...
  const fetchChartData = async () => {
      try {
          ...
          // First, group the extractedPredictionData by date
        ...
        // Now, extract the most recent predicted data for each date
        const mostRecentPredictions = Object.values(groupedByDate).map(group => group[group.length - 1]);

        // Now, use this 'mostRecentPredictions' for your further processing
        const integratedData = mostRecentPredictions.map((pred) => {
            const matchingTemperatureRecord = actualData.find(
                (record) => record.date === pred.timestamp && record.type === 'TEMPERATURE'
            );

            const freqTemperature = matchingTemperatureRecord ? 
                                    matchingTemperatureRecord.value : null;

            return {
                timestamp: pred.timestamp,
                value: pred.value,
                allDataValue: freqTemperature,
            };
    ...
    if (active && payload) {
      ...
      return (
          <div style={{
              ...
              <p style={{ margin: 0, 
                          color: predictedReadingColor }}
                >
                            {`Predicted Reading: ${predictedReading}`}
              </p>
              <p style={{ margin: 0, 
                          color: actualReadingColor }}
                >
