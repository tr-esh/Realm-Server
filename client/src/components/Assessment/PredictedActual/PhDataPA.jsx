...
const PhDataPA = () => {
  ...
  const predictedReadingColor = "#F5D087"; 
  ...
  const fetchChartData = async () => {
      try {
        ...
        const integratedData = mostRecentPredictions.map((pred) => {
            const matchingPhRecord = actualData.find(
                (record) => record.date === pred.timestamp && record.type === 'PH'
            );

            const freqPH = matchingPhRecord ? 
                                    matchingPhRecord.value : null;
