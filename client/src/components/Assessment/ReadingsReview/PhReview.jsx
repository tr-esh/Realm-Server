...
const PhReview = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const responsePredict = await fetch('/api/realm/predictnext/ph');
      ...
      for (const days of daysAgo) {
        ...
        if (!phDataSet[targetDateKey]) {
          phDataSet[targetDateKey] = {
            prediction: null,
            actual: null,
            accuracy: null,
          };
        ...
        if (matchedPredictData) {
          phDataSet[targetDateKey].prediction = matchedPredictData.values.value;
        }
      
        // Matching Actual Data
        const matchedActualData = jsonDataActual
          .filter(item => item.type === 'PH' && fetchDateFromTimestamp(item.date) === fetchDateFromTimestamp(targetDate.toISOString()))
          .sort((a, b) => new Date(b.date) - new Date(a.date))
          .shift(); // Select the latest actual data
      
        if (matchedActualData) {
          phDataSet[targetDateKey].actual = matchedActualData.value;
        }
        ...
        if (matchedAccuracyData) {
          phDataSet[targetDateKey].accuracy = matchedAccuracyData.pH;
        }
      }
  
      const sortedDataSet = Object.entries(phDataSet)
