...
const PredictedValue = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const daysAgo = [1, 2, 3, 4, 5];  // This gives the desired order.
      let tempDataSet = {};
      ...
      const responsePredict = await fetch('/api/realm/predictnext/temperature');
      ...
      for (const days of daysAgo) {
        ...
        if (!tempDataSet[targetDateKey]) {
          tempDataSet[targetDateKey] = {
            prediction: null,
            actual: null,
            accuracy: null,
          };
        ...
        if (matchedPredictData) {
          tempDataSet[targetDateKey].prediction = matchedPredictData.values.value;
        }
      
        // Matching Actual Data
        const matchedActualData = jsonDataActual
          .filter(item => item.type === 'TEMPERATURE' && fetchDateFromTimestamp(item.date) === fetchDateFromTimestamp(targetDate.toISOString()))
          .sort((a, b) => new Date(b.date) - new Date(a.date))
          .shift(); // Select the latest actual data
      
        if (matchedActualData) {
          tempDataSet[targetDateKey].actual = matchedActualData.value;
        }
        ...
        if (matchedAccuracyData) {
          tempDataSet[targetDateKey].accuracy = matchedAccuracyData.temperature;
        }
      }
  
      const sortedDataSet = Object.entries(tempDataSet)
    ...
    {Object.keys(dataSet).length > 0 ? (
      ...
      >
        ...
        {Object.entries(dataSet).map(([date, data], index) => (
          ...
          >
            <BookmarkAddedRoundedIcon style={{fontSize: '1.5rem', 
                                              color: '#8cacff', 
                                              marginLeft: '0.5rem'}}
              />
            <div style={{ flex: 0.9 }}>
            ...
            <div style={{ flex: 1 }} className= 'accuracy-style'>
              {data.accuracy ? `${parseFloat(data.accuracy).toFixed(2)}%` : 'N/A'}
            </div>
          </div>
        ))}
      </div>
